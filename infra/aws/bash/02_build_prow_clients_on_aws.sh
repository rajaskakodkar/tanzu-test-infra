...
chmod 755 "${REPO_PATH}/infra/aws/utils/utils.sh"
...
chmod 755 "${REPO_PATH}/infra/aws/utils/install-jq.sh"
chmod 755 "${REPO_PATH}/infra/aws/utils/install-dependencies.sh"
...
function create_management_cluster {
    ...
    export ENABLE_AUTOSCALER="${MGMT_ENABLE_AUTOSCALER}"
...
function create_service_cluster {
    ...
    export ENABLE_AUTOSCALER="${SERVICE_ENABLE_AUTOSCALER}"
...
function create_build_cluster {
    ...
    export ENABLE_AUTOSCALER="${BUILD_ENABLE_AUTOSCALER}"
    tanzu cluster create "${CLUSTER_NAME}" -f "${REPO_PATH}"/infra/aws/configs/cluster_config.yaml || {
...
function install_prow_on_service_cluster {
    ...
    go run ./gencred --context="${SERVICE_CLUSTER_NAME}"-admin@"${SERVICE_CLUSTER_NAME}" --name="${SERVICE_CLUSTER_NAME}"-trusted --output="${KUBECONFIG_PATH}"
    go run ./gencred --context="${SERVICE_CLUSTER_NAME}"-admin@"${SERVICE_CLUSTER_NAME}" --name=default --output="${KUBECONFIG_PATH}"
  ...
  --clusterrole=cluster-admin --user="${USER}" || {
    ...
    if [ "$USE_EXTERNAL_SECRETS" = false ]; then
      ...
      kubectl -n prow apply -f "${REPO_PATH}"/infra/configs/secrets/aws-secrets-sa.yaml
      kubectl -n test-pods apply -f "${REPO_PATH}"/infra/configs/secrets/aws-secrets-sa.yaml
      kubectl -n prow apply -f "${REPO_PATH}"/infra/configs/secrets/secret-store.yaml
      kubectl -n prow apply -f "${REPO_PATH}"/infra/configs/secrets/external-secrets-prow.yaml
      kubectl -n prow apply -f "${REPO_PATH}"/infra/configs/secrets/external-secrets-test-pods.yaml
...
function install_base_packages_on_service_cluster {
  ...
  chmod 755 "${REPO_PATH}"/infra/aws/utils/add-tce-package-repo.sh
  ...
  kubectl apply -f "${REPO_PATH}"/infra/aws/configs/monitoring-sc.yaml
  ...
  --namespace prometheus || {
    error "UNEXPECTED FAILURE OCCURRED INSTALLING PROMETHEUS!"
    exit 1
  }
  ...
  --namespace grafana || {
    error "UNEXPECTED FAILURE OCCURRED INSTALLING GRAFANA!"
    exit 1
  }
