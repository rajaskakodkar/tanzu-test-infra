...
function collect_management_and_service_cluster_diagnostics {
    cluster_infra=$1
    management_cluster_name=$2
    service_cluster_name=$3
    ...
    if [[ -z "${service_cluster_name}" ]]; then
        error "Service cluster name not passed to collect_management_and_workload_cluster_diagnostics function. Ignoring error. Usage example: collect_management_and_workload_cluster_diagnostics vsphere management-cluster-1234 workload-cluster-1234"
    fi

    echo "Collecting ${management_cluster_name} management cluster and ${service_cluster_name} service cluster diagnostics data"
    ...
    tanzu diagnostics collect --bootstrap-cluster-skip \
        ...
        --workload-cluster-name "${service_cluster_name}" || {
        error "There was an error collecting tanzu diagnostics data. Ignoring the error"
    }
}
